...
class CookieMonster:
    ...
    def cookie_file_check(self):

        cookie = self.config_path / self.cookie_filename
        log.debug('Cookie File Check - Path + Cookie = \"{}\"'.format(cookie))

        try:
            log.debug('Cookie File Check - Checking if cookie exists.')
            if cookie.exists():
                cookie = cookie.as_posix()
                log.debug('Cookie File Check - Cookie Posix Path = \"{}\"'.format(cookie))
                ...
                log.debug('Cookie File Check - Cookie file does not exist.')
        except:
            log.debug('Cookie File Check - Fail.')

        return False

    def cookie_validity_check(self):
        ...
        for key in self.r.cookies.get_dict(domain=self.url.cookie_domain).keys():
            if key in ['1P_JAR', 'APISID', 'CONSENT', 'HSID', 'NID', 'SAPISID', 'SID', 'SIDCC', 'SSID']:
                ...
                log.debug('Cookie Validity Check - Failed cookie validation.')
                return False
    ...
    def init_browser(self):
        ...
        try:
            ...
            log.error('Driver - Browser initialization failed.')
    ...
    def run(self):
        ...
        if self.cookie_file_check() and self.cookie_validity_check():
