...
class CookieMonster:
    ...
    def cookie_check(self):
        ...
        try:
            ...
            with open(self.cookie_location, mode='rb') as f:
    ...
    def login(self):
        ...
        # TODO: add try with os for debug folder. create if doesn't exist.

        def debug(browser, output, ss, url):
            browser.save_screenshot('{}/debug/{}'.format(output, ss))
            with open('{}/debug/urls.txt'.format(output), mode='a') as url_output:
                url_output.write('{} URL: {}\n'.format(url, browser.current_url))

        log.debug('Login - Opening {}.'.format(self.login_url))
        self.browser.get(self.login_url)

        if self.debug:
            debug(self.browser, self.cookie_location, 'gmail_login_01.png', 'Login Screen')
        ...
        if self.debug:
            debug(self.browser, self.cookie_location, 'gmail_login_02.png', 'Email Submission')

        log.debug('Login - Submitting email.')
        email.send_keys(Keys.RETURN)

        if self.debug:
            debug(self.browser, self.cookie_location, 'gmail_login_03.png', 'Password Screen')
        ...
        if self.debug:
            debug(self.browser, self.cookie_location, 'gmail_login_04.png', 'Password Submission')

        log.debug('Login - Submitting password.')
        password.send_keys(Keys.RETURN)

        if self.debug:
            debug(self.browser, self.cookie_location, 'gmail_login_05.png', '2FA Wait')
        ...
        if self.debug:
            debug(self.browser, self.cookie_location, 'gmail_login_06.png', 'Account Home')
    ...
    def export_cookies(self):
        ...
        with open(self.cookie_location, mode='wb') as f:
