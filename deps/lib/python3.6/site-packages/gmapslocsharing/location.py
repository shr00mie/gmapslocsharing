...
class LocationSharing:
    ...
    def query_data(self):
        ...
        self.output_raw = None
        # TODO: need to put a try in here.
        # it's more for the decode component as i've noticed that it sometimes
        # fails. don't think i've had the query itself fail yet.
    ...
    def clean_data(self):
        ...
        log.debug('Data Cleanup - Clearing previously clean output')
        self.output_clean = []

        log.debug('Data Cleanup - Moving data to local variable.')
        output_dirty = self.output_raw
        ...
        output_dirty = output_dirty.split(sep='[[')[2:]

        log.debug('Data Cleanup - Deleting useless/unknown content.')
        people = []
        for person in output_dirty:
        ...
        for person in people:
            ...
            try:
                ...
                log.info('Data Cleanup - Issue reading battery info for {}.'.format(person_data['first_name']))
                ...
                with open(self.debug_file, mode='a') as debug_output:
                    debug_output.write('[{}]\n{}\n\n'.format(timestamp, self.output_raw))

                log.info('Data Cleanup - Setting battery level to 50% and charging to false due to report error.')
                person_data['battery'] = {  'charging': False,
                                            'percent': 50}
