...
class CookieMonster:
    ...
    def __init__(self,  login, password, config, path, cookie, resolution,
        ...
        self.agent = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{} Safari/537.36'.format(CHROME_VERSION)
        self.header =   {
    ...
    def validity_check(self):
        ...
        log.debug('Validating cookie contents.')
        ref = ['1P_JAR','APISID','CONSENT','HSID','NID','SAPISID',
                            'SID','SIDCC','SSID','GAPS','OSID','OTZ']
        keys = [_ for _ in self.session.cookies.get_dict().keys()]
        if ref != keys:
            return False

        log.debug('Checking cookie expiration.')
        cdate = self.session.cookies.get_dict()['1P_JAR']
        cdt = datetime.strptime(cdate, '%Y-%m-%d-%H')
        cdt = pytz.timezone('UTC').localize(cdt)
        cdays = round((time() - cdt.timestamp()) / 60 / 60 / 24, 2)

        if cdays >= 23:
            log.warning('Cookie will expire in 7 days, recreating.')
            self.cookie.touch(mode=0o660, exist_ok=True)
            return False
